...
std::vector<Geometry::LineSegment2D> buildContourSegments(const Sampling::ScalarLattice& lattice,
    ...
    for (int iy = 0; iy < lattice.cellsY; ++iy) {
        ...
        for (int ix = 0; ix < lattice.cellsX; ++ix) {
            ...
            if (interpolateIso(x0, y0, v0, x1, y0, v1, isoValue, intersections[count])) {
                ++count;
            }
            if (interpolateIso(x1, y0, v1, x1, y1, v2, isoValue, intersections[count])) {
                ++count;
            }
            if (interpolateIso(x1, y1, v2, x0, y1, v3, isoValue, intersections[count])) {
                ++count;
            }
            if (interpolateIso(x0, y1, v3, x0, y0, v0, isoValue, intersections[count])) {
                ++count;
            }

            if (count == 2) {
                segments.push_back(Geometry::LineSegment2D{ intersections[0], intersections[1] });
            } else if (count == 4) {
                segments.push_back(Geometry::LineSegment2D{ intersections[0], intersections[1] });
                segments.push_back(Geometry::LineSegment2D{ intersections[2], intersections[3] });
            }
        }
    }
